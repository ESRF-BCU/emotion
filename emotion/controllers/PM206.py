...
"""
Emotion controller for PiezoMotor PM206 piezo motor controller.
Ethernet
...
class PM206(Controller):
    ...
    def __init__(self, name, config, axes):
        ...
        self._motor_error_codes = [
            ...
            (0x10, "Max or min encoder limit was reached in target mode, or motor was stopped due to external limit signal "),
            (0x8, "Target mode is active"),
            (0x4, "Target position was reached (if target mode is active). Also set during parking/unparking. "),
            (0x2, "Motor direction"),
    ...
    def read_position(self, axis, measured=False):
        ...
        if measured:
            ...
            _pos = int(_ans[8:], 16)
            pm206_debug( "PM206 position measured read : %d (_ans=%s)" % (_pos, _ans))
        else:
            _ans = self.send(axis, "TP?")
            _pos = int(_ans[8:], 16)
            pm206_debug( "PM206 position setpoint read : %d (_ans=%s)" % (_pos, _ans))
    ...
    def read_velocity(self, axis):
        ...
        pm206_debug ("read_velocity : %d" % _velocity)
        return _velocity
    ...
    def set_velocity(self, axis, new_velocity):
        ...
        pm206_debug("velocity wrotten : %d " % _nv)

        return self.read_velocity(axis)



    def pm206_get_status(self, axis):
        '''
        ...
        pm206_debug("ctrl status : %s" % self._ctrl_status)
        pm206_debug("mot1 status : %s" % self._axes_status[1])
        pm206_debug("mot2 status : %s" % self._axes_status[2])
        pm206_debug("mot3 status : %s" % self._axes_status[3])
        pm206_debug("mot4 status : %s" % self._axes_status[4])
        pm206_debug("mot5 status : %s" % self._axes_status[5])
        pm206_debug("mot6 status : %s" % self._axes_status[6])
    ...
    def motor_state(self, axis):
        ...
        pm206_debug("axis %d status : %s" % (axis.channel, _axis_status))
    ...
    """
    PM206 specific communication
    """
    def send(self, axis, cmd):
        """
        - Adds 'CP<x><y>' prefix
        - Adds the 'carriage return' terminator character : "\\\\r"
        - Sends command <cmd> to the PM206 controller.
        ...
        pm206_debug("send(%s) returns : %s " % (_cmd, _ans))
        ...
        if _duration > 0.005:
            print "PM206 Received %s from Send %s (duration : %g ms) " % (repr(_ans), _cmd, _duration * 1000)
